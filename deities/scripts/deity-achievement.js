...
const DEITIES_DATA_PATH = "../data/deities.json";
...
const loadDeityNames = async () => {
  if (deityLoadPromise) return deityLoadPromise;
  deityLoadPromise = (async () => {
    try {
      const response = await fetch(DEITIES_DATA_PATH, { cache: "no-store" });
      if (!response.ok) throw new Error("Failed to load deity data");
      const list = await response.json();
      const slug = getSlug();
      const data = list.find((item) => item.slug === slug);
      if (!data) return null;
      const enName = data?.name?.en || "";
      const zhName = data?.name?.zh || "";
      return { enName, zhName };
    } catch (error) {
      console.error("Failed to load deity info", error);
      return null;
    }
  })();
  return deityLoadPromise;
};
...
const checkReligionMatch = async () => {
  ...
  const deityInfo = await loadDeityNames();
  if (!deityInfo) return;
  const normalizedReligion = normalizeText(religionValue);
  const matchesEn = deityInfo.enName && normalizedReligion.includes(normalizeText(deityInfo.enName));
  const matchesZh = deityInfo.zhName && normalizedReligion.includes(normalizeText(deityInfo.zhName));
